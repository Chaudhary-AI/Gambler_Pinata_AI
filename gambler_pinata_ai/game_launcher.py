...
POSTER_PATH = os.path.join(TEMPLATE_FOLDER, "game_poster_start")
START_BUTTON_PATH = os.path.join(TEMPLATE_FOLDER, "game_start_button")
SCATTER_PATH = os.path.join(TEMPLATE_FOLDER, "scatter_template")
...
def launch_game():
    ...
    while True:
        pos = detect_element("game_poster_start", 0.75)
        if pos:
    ...
    while True:
        pos = detect_element("game_start_button", 0.75)
        if pos:
            click_at(pos)
    ...
    for _ in range(10):
        if detect_element("scatter_template", 0.7):
